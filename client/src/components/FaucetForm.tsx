          ...
          {!sendTokenResponse.txHash ? (
            <div>
              <p className="rate-limit-text">
                ...
                <span>
                  {chainConfigs[chain!]?.RATELIMIT?.MAX_LIMIT} request in{" "}
                  {toString(chainConfigs[chain!]?.RATELIMIT?.WINDOW_SIZE)}.
                </span>
              </p>
              ...
              >
                {isLoading ? (
                  ...
                  <span>
                    Request {chainConfigs[chain || 0]?.DRIP_AMOUNT / 1e9}{" "}
                    {chainConfigs[chain || 0]?.TOKEN}
                  </span>
                )}
              </button>
            </div>
